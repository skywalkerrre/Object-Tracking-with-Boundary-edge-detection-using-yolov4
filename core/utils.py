...
def draw_bbox(image, bboxes, info = False, counted_classes = None, show_label=True, allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), read_plate = False):
    ...
    for i in range(num_boxes):
        ...
        if class_name not in allowed_classes:
            ...
            # this is for images where CTB is not required
            """ if info:
                print("Object found: {}, Confidence: {:.2f}, BBox Coords (xmin, ymin, xmax, ymax): {}, {}, {}, {}".format(class_name, score, coor[0], coor[1], coor[2], coor[3]))
  """
            # this is for videos where CTB is required
            if info:
                if(int(coor[0])>550 or int(coor[0])<20 or int(coor[1])<20):
                    print("Object found: {}, Confidence: {:.2f}, BBox Coords (xmin, ymin, xmax, ymax): {}, {}, {}, {}, Close To Boundary ".format(class_name, score, coor[0], coor[1], coor[2], coor[3]))
                else:
                    print("Object found: {}, Confidence: {:.2f}, BBox Coords (xmin, ymin, xmax, ymax): {}, {}, {}, {}".format(class_name, score, coor[0], coor[1], coor[2], coor[3]))
  ...
  """
            # this is for videos where CTB is required
            if show_label:
                if (int(coor[0])>540 or int(coor[0])<10 or int(coor[1])<20):
                    hsv_tuples = [(3.0 * x / num_classes, 3., 3.) for x in range(num_classes)]
                    colors = list(map(lambda x: colorsys.hsv_to_rgb(*x), hsv_tuples))
                    colors = list(map(lambda x: (int(x[0] * 255), int(x[1] * 255), int(x[2] * 255)), colors))
                    bbox_mess = '%s: %.2f: %s' % (class_name, score, 'CTB!!!')
                    t_size = cv2.getTextSize(bbox_mess, 0, fontScale, thickness=bbox_thick // 2)[0]
                    c3 = (c1[0] + t_size[0], c1[1] - t_size[1] - 3)
                    cv2.rectangle(image, c1, (int(np.float32(c3[0])), int(np.float32(c3[1]))), bbox_color, -1)

                    cv2.putText(image, bbox_mess, (c1[0], (c1[1] - 2)), cv2.FONT_HERSHEY_SIMPLEX, fontScale, (0, 0, 0), bbox_thick // 2, lineType=cv2.INTER_CUBIC)
